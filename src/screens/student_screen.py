...
def student_dashboard():
    ...
    with c2:
        ...
        if st.button("Logout", type ="secondary",key = "student_loginbackbtn",shortcut ="control+backspace"):
...
def student_screen():
    ...
    if photo_source is None and not st.session_state.get("show_registration", False):

        st.stop()   
    if photo_source:
        img = np.array(Image.open(photo_source))
        st.session_state["captured_face"] = img

        with st.spinner("AI is scanning.."):
            detected,all_ids,num_faces =predict_attendance(img)
            # st.write(f"DEBUG → detected: {detected}, num_faces: {num_faces}")
            if num_faces == 0:
    ...
    if st.session_state["show_registration"]:
        with st.container(border= True):
            ...
            audio_data = None
            try:
                audio_data = st.audio_input("Record a short phrase like, I am present, My name is Anuj")
            except Exception:
            ...
            if  st.button("Create Account",type ="primary"):
                if new_name:
                    with st.spinner("Creating Profile.."):
                        img = st.session_state["captured_face"]
